...
for i in range(0,rows):
    ...
    for j in range(1,11):   
        ...
        row.append(df.loc[i][j]) #có 10 phần tử trên 1 row
print(list_row) #danh sách gồm 10*3 rows phần tử trong tập dữ liệu ban đầu
num_sample_row=len(row)
n_size = num_sample_row * 20 + 1

list_train=[]

for i in range(0,n_size):
    new_row = resample(list_row, replace=True, n_samples=num_sample_row)
