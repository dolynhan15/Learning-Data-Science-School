...
# đếm số lần xuất hiện của giá trị ở cột zip
top5zip = df['zip'].value_counts().head(5)    # use method value_counts() it give top 5 highest
print(top5zip)
# split là hàm dùng khi biến thuộc kiểu dữ liệu string
# lambda là hàm thay thế cho list
df['Reason'] = df['title'].apply(lambda title: title.split(':')[0])
#print(df['Reason'])
df['Reason'].value_counts()

# Vẽ biểu đồ tần suất hiển thị số lần của giá trị xh
sns.countplot(x='Reason',data=df,palette='viridis')
plt.show()

df['timeStamp'] = pd.to_datetime(df['timeStamp'])
# time.hour là hàm có sẵn dùng khi biến thuộc kiểu dữ liệu datetime
